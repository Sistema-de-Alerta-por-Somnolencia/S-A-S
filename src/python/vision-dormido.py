    ...
    with FaceLandmarker.create_from_options(options) as landmarker:
        ...
        while cap.isOpened():
            ...
            if detection_result.face_blendshapes:
                ...
                ojos_cerrados = scores.get("eyeBlinkLeft", 0) > 0.5 and scores.get("eyeBlinkRight", 0) > 0.5

                if ojos_cerrados:
                    if tiempo_inicio_ojos_cerrados is None:
                        tiempo_inicio_ojos_cerrados = time.time()
                    
                    if (time.time() - tiempo_inicio_ojos_cerrados) >= TIEMPO_LIMITE_SEGUNDOS and not alerta_ya_enviada:
                        ...
                        threading.Thread(target=enviar_json_camiones, args=(datos,), daemon=True).start()
                        threading.Thread(target=enviar_correos_dinamicos, args=(datos,), daemon=True).start()
                        threading.Thread(target=hacer_sonar_alarma, args=("src/public/img/alerta.wav",), daemon=True).start()
                else:
                    tiempo_inicio_ojos_cerrados = None
                    alerta_ya_enviada = False
                ...
                cv2.putText(frame_final, txt, (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, col, 3)
