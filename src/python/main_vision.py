...
class VisionSystem:
    ...
    def worker_drowsiness(self):
        ...
        with mp.tasks.vision.FaceLandmarker.create_from_options(self.options_mp) as landmarker:
            while self.running:
                with self.lock:
                ...
                if results.face_blendshapes:
                    ...
                    for bs in face_blendshapes:
                        if bs.category_name == "eyeBlinkLeft": blink_left = bs.score
                        elif bs.category_name == "eyeBlinkRight": blink_right = bs.score
                        elif bs.category_name == "jawOpen": boca_abierta = bs.score
                    ...
                    if blink_left > 0.5 and blink_right > 0.5:
                        ...
                        tiempo_transcurrido = time.time() - self.tiempo_inicio_ojos_cerrados
                        if tiempo_transcurrido >= self.TIEMPO_LIMITE_SEGUNDOS and not self.alerta_ya_enviada:
    ...
    def worker_face_rec(self):
        ...
        while self.running:
            if self.vector_conocido is None: break
            ...
            if len(face_encodings) > 0:
                match = fr.compare_faces([self.vector_conocido], face_encodings[0], tolerance=0.5)
        ...
        while True:
            ...
            if self.yolo_results is not None:
                ...
                for box in self.yolo_results.boxes:
                    ...
                    cv2.putText(display_frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
            ...
            text_size = cv2.getTextSize(self.alert_text, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)[0]
            cv2.putText(display_frame, self.alert_text, ((w - text_size[0]) // 2, 40), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, self.alert_color, 2)
            
            # Chofer (Izquierda)
            cv2.putText(display_frame, f"Chofer: {self.driver_name}", (10, 40), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

            # FPS o Status (Derecha)
            cv2.putText(display_frame, "SAS v1.0 | UAM", (w - 180, 40), 
